...
def create_tf_dataloader(file_dict, data_config_file):
    ...
    pytorch_dataloader = SimpleIterDataset(file_dict=file_dict, data_config_file=data_config_file, fetch_step=0.0045)
    ...
    tf_dataloader = tf.data.Dataset.from_generator(lambda: (process_pytorch_data(data) for data in pytorch_dataloader),output_signature = (
    # tf_dataloader = tf.data.Dataset.from_generator(lambda: (process_pytorch_data(data) for data in islice(pytorch_dataloader, 0, 96 * 20)),output_signature = (
        #tf.TensorSpec(shape=(128, 2), dtype=tf.float32),
        tf.TensorSpec(shape=(17, 128), dtype=tf.float32),
        tf.TensorSpec(shape=(4, 128), dtype=tf.float32),
